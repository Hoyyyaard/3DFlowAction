...
def train(cfg: DictConfig):
    ...
    for epoch in tqdm(range(cfg.training.num_train_epochs)):
        model.train()
        epoch_loss = []
        data_bar = tqdm(total=len(train_dataloader), desc=f"Epoch {epoch}")
        for step, batch in enumerate(train_dataloader):
            ...
            with torch.no_grad():
                if os.environ.get("USE_VAE") == "True":
                    ...
                    latents = latents.sample()   # [bs*32, 4, 4, 4]
                    latents = rearrange(latents, "(b f) c h w -> b c f h w", f=video_length) # c = 4 [bs, 4, 32, 4, 4]
            ...
            if accelerator.is_main_process and step % cfg.training.log_frequency == 0:
                ...
                print(f"######### [Epoch] {epoch}, [Step] {step}, [Loss] {loss.item()} [Memory] {torch.cuda.memory_allocated() / 1024**3}GB ########")
        ...
        if epoch % cfg.evaluation.eval_frequency == 0 or cfg.debug:
            accelerator.wait_for_everyone()
