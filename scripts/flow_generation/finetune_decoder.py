...
from im2flow2act.flow_generation.ae_inference import ae_flow_inference, ae_flow_inference_3d
...
def train(cfg: DictConfig):
    ...
    if hasattr(cfg, "resume_model_path"):
        vae = AutoencoderKL.from_pretrained(cfg.resume_model_path)
        start_epoch = int(cfg.resume_model_path.split("/")[-1].split("_")[-1])
        print(">>> Resuming from path", cfg.resume_model_path)
    else:
    ...
    pbar = tqdm.tqdm(total=len(range(cfg.training.num_train_epochs)[start_epoch:]), desc="Epoch")
    for epoch in range(cfg.training.num_train_epochs)[start_epoch:]:
        pbar.update(1)
        vae.train()
        epoch_loss = []
        # tqdm_iter = tqdm.tqdm(total=len(train_dataloader), desc="Iteration")
        ...
        if accelerator.is_main_process and not cfg.debug :
            ...
            print(f"Epoch {epoch} loss: {np.mean(epoch_loss)} memory: {torch.cuda.max_memory_reserved(0) / (1024**3)}GB")
        ...
        if epoch % cfg.evaluation.eval_frequency == 0 and epoch > 0:
            ...
            if accelerator.is_local_main_process:
                ...
                for dth, eval_dataset in enumerate(evalulation_datasets):
                    ...
                    if hasattr(cfg.evaluation, "3D"):
                        ae_flow_inference_3d(
                            cfg.evaluation,
                            eval_vae,
                            eval_dataset,
                            wandb_log=False,
